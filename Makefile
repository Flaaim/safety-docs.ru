...
api-clear:
	docker run --rm -v ${PWD}/api:/app -w /app alpine sh -c 'rm -rf var/cache/* var/log/* var/test/* public/templates/* public/distributions/import/*'

api-permission:
	docker run --rm -v ${PWD}/api:/app -w /app alpine chmod -R 777 bin var/cache var/log var/test public/templates public/distributions
...
deploy:
	...
	ssh ${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER} && docker compose run --rm api-php-cli composer app migrations:migrate'
