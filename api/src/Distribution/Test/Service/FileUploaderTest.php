...
final class FileUploaderTest extends TestCase
{
    ...
    public function testClientFileNameNotExists(): void
    {
        $uploadFile = $this->createMock(UploadedFileInterface::class);
        $uploadFile->expects(self::once())->method('getClientFilename')->willReturn(null);

        self::expectException(\DomainException::class);
        self::expectExceptionMessage('File name cannot be null');
        $this->handler->upload('error_file.txt', $uploadFile);
    }
    ...
    {
        $uploadFile = $this->createMock(UploadedFileInterface::class);
        $uploadFile->expects(self::once())->method('getClientFilename')->willReturn('text.csv');

        $expectedPath = 'vfs://storage/directory/text.csv';
